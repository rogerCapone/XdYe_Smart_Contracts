...
contract XdYeMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _xdye, address _weth) public {
        factory = _factory;
        xdye = _xdye;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, xdye), amount);
            return amount;
        }
        ...
        _safeTransfer(weth, factory.getPair(weth, xdye), amountOut);
        return amountOut;
    }
